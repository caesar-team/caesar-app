...
final class VectorsTests: XCTestCase {
    ...
    /// Reads the canonical vectors — no vendored copy exists, so the two SDKs cannot drift.
    /// Walking up from `#filePath` (rather than counting `../`) keeps this working if the
    /// package moves inside the monorepo.
    private func vectorsURL() throws -> URL {
        ...
        throw XCTSkip("\(Self.canonicalPath) not found — is this a full monorepo checkout?")
    }
